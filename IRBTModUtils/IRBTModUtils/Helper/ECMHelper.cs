...
namespace LowVisibility.Helper {
    ...
    class ECMHelper {
        ...
        public static void UpdateECMState(AbstractActor source) {
            ...
            if (jamming > 0) {
                Mod.Log.LogIfTrace($"  -- target:{CombatantHelper.Label(source)} has ECM jamming:{jamming}");
            ...
            if (protection > 0) {
                Mod.Log.LogIfTrace($"  -- target:{CombatantHelper.Label(source)} has ECM protection:{protection}");
            ...
            List<Effect> narcEffects = allEffects != null
                ? allEffects.Where(e => e?.EffectData?.tagData?.tagList != null)
                    ...
                    .ToList()
                : new List<Effect>();
            Mod.Log.LogIfTrace($"  -- target:{CombatantHelper.Label(source)} has:{(narcEffects != null ? narcEffects.Count : 0)} NARC effects");
            int narcEffect = 0;
                ...
                if (effectTag != null) {
                    ...
                    if (split.Length == 2) {
                        ...
                        if (modifier > narcEffect) {
                            narcEffect = modifier;
                            Mod.Log.LogIfDebug($"  Effect:{effect.EffectData.Description.Id} adding modifier:{modifier}.");
                        }
                    } else {
                        Mod.Log.Log($"Actor:{CombatantHelper.Label(source)} - MALFORMED EFFECT TAG -:{effect}");
                    }
                }
            }
            if (narcEffect != 0) {
                Mod.Log.LogIfDebug($"  -- target:{CombatantHelper.Label(source)} has NARC beacon with value:{narcEffect}");
                if (protection >= narcEffect) {
                    Mod.Log.LogIfDebug($"  -- target:{CombatantHelper.Label(source)} has NARC beacon mod:{narcEffect} " +
                    ...
                    Mod.Log.LogIfDebug($"  -- target:{CombatantHelper.Label(source)} has NARC beacon with modifier:{narcEffect} " +
            ...
            List<Effect> tagEffects = allEffects != null
                ? allEffects.Where(e => e?.EffectData?.tagData?.tagList != null)
                    ...
                    .ToList()
                    : new List<Effect>();
            Mod.Log.LogIfTrace($"  -- target:{CombatantHelper.Label(source)} has:{(tagEffects != null ? tagEffects.Count : 0)} TAG effects");
            int tagEffect = 0;
            ...
            if (tagEffect != 0) {
                ...
                Mod.Log.LogIfDebug($"  -- target:{CombatantHelper.Label(source)} has TAG effect with value:{tagEffect}");
            } else {
        ...
        private static int CalculateECMStrength(AbstractActor target, List<AbstractActor> sources) {
            ...
            foreach (AbstractActor actor in sources) {
                ...
                if (actorStaticState.ecmMod != 0 && actorsDistance <= actorStaticState.ecmRange) {
                    Mod.Log.LogIfDebug($"Target:{CombatantHelper.Label(target)} and ECM source:{CombatantHelper.Label(actor)} are {actorsDistance}m apart, " +
            ...
            if (ecmSourceCount > 1) {
                ...
                Mod.Log.LogIfDebug($"Target:{CombatantHelper.Label(target)} has:{ecmSourceCount} ECM sources within range. " +
